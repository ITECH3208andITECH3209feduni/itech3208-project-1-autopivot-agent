// AutoPivot frontend controller.
// Handles uploads, processing options, API calls, previews, progress, and resets.
...
{
  if (backgroundStyleSelect.value !== 'custom')
  {
    bgFileInput.value = '';
  }
...
{
  bgFileInput.value = '';
  backgroundStyleSelect.value = 'studio_full';
  updateBackgroundControl();
  resetProcessedResult();
  updateResetButtons();
}
...
async function executePipeline() 
{
  ...
  if (endpoint === '/process-vehicle' || endpoint === '/remove-background') 
  {
    ...
    if (backgroundStyle === 'custom')
    {
      ...
      {
        throw new Error('Select a custom background image, or choose an AutoPivot studio preset.');
      }
...
function renderStats(json, endpoint)
{
  processingStats.innerHTML = '';
  const rows = [];
  const modelName = json.bg_model_used ? json.bg_model_used.split('/').pop() : null;

  if (endpoint === '/process-vehicle' && json.vehicle)
  {
    rows.push(['Vehicle', `${json.vehicle.class} (${(json.vehicle.score * 100).toFixed(1)}%)`]);
    rows.push(['Plates detected', `${json.plates_detected} (${json.plate_treatment})`]);
  }
  ...
  if (modelName) rows.push(['Background model', modelName]);
  if (endpoint !== '/detect-and-hide')
  {
    rows.push(['Background', json.background || 'Transparent']);
    if (json.background_style !== 'transparent')
    {
      rows.push(['Showroom matching', json.colour_matched ? 'Applied' : 'Not used']);
      rows.push(['Ground shadow', json.shadow_applied ? 'Applied' : 'Not used']);
    }
  }

  processingStats.innerHTML = rows
    .map(([label, value]) => `<div class="stats-row"><span>${label}:</span> <strong>${value}</strong></div>`)
    .join('');
  processingStats.style.display = rows.length ? 'block' : 'none';
}
...
function setLoading(on) 
{
  ...
  backgroundStyleSelect.disabled = on;
...
{
  const isCustom = backgroundStyleSelect.value === 'custom';
  customBackgroundUpload.style.display = isCustom ? 'block' : 'none';
}

function updateResetButtons(isLoading = false) 
{
  const backgroundChanged = backgroundStyleSelect.value !== 'studio_full' || bgFileInput.files.length > 0;
  resetCarButton.disabled = isLoading || !currentFile;
  resetBgButton.disabled = isLoading || !backgroundChanged;
  resetPlateButton.disabled = isLoading || !plateFileInput.files.length;
  resetAllButton.disabled = isLoading || (
    !currentFile &&
    !backgroundChanged &&
    !plateFileInput.files.length
  );
}
