...
class PipelineProtocol(Protocol):
    def process_vehicle(self, image, background=None, plate_overlay=None) -> PipelineResult: ...
    def remove_background(self, image, background=None) -> PipelineResult: ...
...
def processing_routes(service: PipelineProtocol, config: Settings) -> APIRouter:
    ...
    async def process_vehicle(
        ...
        return result_payload(service.process_vehicle(image, background_image, plate_image))
    ...
    async def remove_background(
        ...
        image = await read_image(file, config)
        background_image = await read_optional(background, config)
        return result_payload(service.remove_background(image, background_image))
