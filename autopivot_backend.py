...
class PipelineProcessor:
    ...
    def process(
        ...
        background_image = _open_image(background).convert("RGBA") if background else None

        result = _run_realistic_vehicle_pipeline(source, background_image, None)
        if not result["vehicle_detected"]:
            return processing.ProcessOutcome(
                image_png=None,
                vehicle_detected=False,
                message=result.get("message") or "No vehicle detected in this photograph.",
            )
        ...
        return processing.ProcessOutcome(
            ...
            plate_treatment="masked" if result["plates_detected"] else "none",
            model_used=result["bg_model_used"],
        )
...
def _run_segmentation(model, image: Image.Image) -> Image.Image:
    ...
    single-channel sigmoid output that is refined into an alpha mask.
    """
...
async def api_process_vehicle(
    ...
    """
    Full processing pipeline with the measured AutoPivot raised-platform showroom compositor.

    If a custom background is uploaded it is still used, but when no background is
    provided the pipeline falls back to `assets/backgrounds/studio-full.png`.
    """
    content = await file.read()
    _validate_upload(file, content)
    ...
    logger.info(
        ...
        len(content),
        "yes" if bg_image else "default" if app_config.USE_DEFAULT_STUDIO_BACKGROUND else "no",
        "yes" if plate_img else "no",
    )

    result = _run_realistic_vehicle_pipeline(image, bg_image, plate_img)
    if not result["vehicle_detected"]:
        logger.info("No vehicle detected — pipeline aborted")
        return {
            "success": False,
            "vehicle_detected": False,
            "message": result.get("message") or "No vehicle detected. Please upload a clear vehicle image.",
        }

    final = result.pop("image")
    result["success"] = True
    result["processed_image"] = _encode_png(final)
    return result
