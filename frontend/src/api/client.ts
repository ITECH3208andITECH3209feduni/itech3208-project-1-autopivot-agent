...
/**
 * Best-effort read of a listing page — see api/url_import.py on the backend.
 * Every field is null when nothing reliable could be found there, not when
 * the vehicle genuinely has no make; it only ever pre-fills an editable
 * field, so a null here just means the dealer types that one themselves.
 */
...
export const api = {
  ...
  /**
   * A lighter check than importImagesFromUrl: reads make/model/year/etc from
   * a pasted listing URL without downloading its photographs, so it can run
   * before a listing exists yet (as soon as the URL field is filled in).
   */
  previewListingUrl: (url: string) =>
    request<UrlPreviewResult>('/api/listings/preview-url', {
      method: 'POST',
      body: JSON.stringify({ url }),
    }),
