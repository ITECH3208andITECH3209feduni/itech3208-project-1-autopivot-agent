...
export default function UploadView() {
  ...
  // Best-effort auto-fill from the pasted listing URL, fired on blur — see
  // handleUrlBlur. Tracked separately from importError/importNote because a
  // preview that finds nothing is a normal, silent outcome, not a fault to
  // report the way a failed photograph import is.
  ...
  // Fires when the Listing URL field loses focus. A best-effort convenience,
  // not a required step: it reads the page's own details and fills in
  // whichever of make/model/year/variant/stock number are still blank, so a
  // dealer who has already started typing never has their own text
  // overwritten. Failures here are swallowed rather than surfaced through
  // `importError` — that field is reserved for the photograph import, which
  // the dealer explicitly asked for by submitting the form; a preview they
  // didn't ask for failing outright should not read as a problem.
  async function handleUrlBlur() {
    ...
    try {
      ...
      // Best-effort only; the dealer can still type everything by hand.
    } finally {
