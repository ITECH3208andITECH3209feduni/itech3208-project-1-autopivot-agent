...
## Default AutoPivot studio background

If no backdrop is selected, the full ML backend now uses `assets/backgrounds/studio-full.png` as the default showroom and applies the measured raised-platform compositor. Replace that file with your final showroom render while keeping the platform position close to the documented measurements in `PLATFORM_MEASUREMENTS.md`.
