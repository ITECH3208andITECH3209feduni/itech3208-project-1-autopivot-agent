...
class Backdrop(Base):
    ...
    __table_args__ = (
        ...
        # NULL means "not measured yet" — the compositor falls back to its
        # generic guess (APA-138). A non-NULL value has to leave headroom
        # above and below the frame edge, or the vehicle's feet or shadow
        # would be clipped off the canvas.
        CheckConstraint(
            "ground_y_ratio IS NULL OR ground_y_ratio BETWEEN 0.05 AND 0.98",
            name="ground_y_ratio_plausible",
        ),
    )
    ...
    # Where this scene's floor actually is, as a fraction of canvas height —
    # the vehicle's contact point is placed here (APA-138). NULL means nobody
    # has measured it yet: compositing.py falls back to DEALER_BACKDROP's
    # generic 0.84 guess, which is wrong for any scene whose horizon sits
    # somewhere else and is why an unconfigured backdrop shows the vehicle
    # floating or sinking. Only the two built-in studio presets, which live in
    # code rather than this table, ship with an accurate value from the start.
    ground_y_ratio: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, server_default=func.now()
    )
    updated_at: Mapped[datetime] = mapped_column(
