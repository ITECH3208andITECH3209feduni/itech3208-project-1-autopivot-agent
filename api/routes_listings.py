...
async def preview_listing_url(body: UrlImportRequest, user: CurrentUser) -> UrlPreviewResult:
    """
    Best-effort read of make/model/year/etc from a pasted listing URL, before
    any listing exists — lets the "Add a vehicle" form pre-fill itself as soon
    as the dealer pastes a link, rather than only after photographs are
    imported.

    Deliberately lighter than the real import: url_import.fetch_vehicle_details
    stops after reading the page's head, so a slow or huge listing gallery
    does not have to be downloaded just to preview its details. A page that
    fails outright (blocked host, timeout, not HTML) reads the same as one
    that simply has no details — this is only ever a convenience the dealer
    can override, so it degrades to an empty form rather than an error.
    """
    try:
        details = await url_import.fetch_vehicle_details(body.url)
    except url_import.UrlImportError:
        details = None

    if details is None or details.is_empty():
        return UrlPreviewResult(vehicle=None)

    return UrlPreviewResult(
        vehicle=VehicleDetailsOut(
            make=details.make,
            model=details.model,
            year=details.year,
...
async def import_images_from_url(
    ...
    for fetched in result.images[:room]:
        ...
        # storage_path is content-addressed and globally unique per
        # dealership, so this insert collides whenever *this exact file* is
        # already attached to some listing — not necessarily this one. A
        # scraped page brings in more than the vehicle (see url_import's own
        # notes on badges and banners it cannot filter out), and a site that
        # reuses the same logo or finance banner across every listing means
        # that asset gets "claimed" by whichever listing imports it first;
        # every later import that also picks it up hits this collision on a
        # page the dealer has never used before. One bad photo in the batch
        # used to fail the whole request (see git history) — a savepoint per
        # image means a collision here costs one photo, not the import.
        try:
    ...
    if not created:
        raise HTTPException(
            ...
            detail=(
                ...
                "a logo or banner reused across the site's listings, not a "
                "sign this listing was imported before. Try adding photographs "
                "directly instead."
                if duplicates
    ...
    if duplicates:
        already_note = (
            f"{duplicates} photograph{'s' if duplicates != 1 else ''} on that "
            "page were already stored elsewhere (commonly a shared site logo "
            "or banner) and were left out."
        )
    ...
    vehicle = (
        VehicleDetailsOut(
            ...
            stock_number=result.vehicle.stock_number,
        )
        if result.vehicle and not result.vehicle.is_empty()
        else None
    )
